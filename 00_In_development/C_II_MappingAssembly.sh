...
for FQZ in *.fq.gz; do
        ...
        #BWA
        #BWA-aln
        bwa aln -t 40 $REF_BWA $FQZ \
        | bwa samse $REF_BWA - $(basename $FQZ)\
        | samtools view -uSh - \
        | samtools sort - -o $(basename "${FQZ/\.fq\.gz/}")_MAPPING_bwaaln.sort.bam

        #BWA-aDNA
        bwa aln -t 40 -l 1024 -n 0.01 -o 2 $REF_BWA $FQZ \
        | bwa samse $REF_BWA - $(basename $FQZ)\
        | samtools view -uSh - \
        | samtools sort - -o $(basename "${FQZ/\.fq\.gz/}")_MAPPING_bwaaDNA.sort.bam

        #BWA-mem
        bwa mem -t 40 $REF_BWA $FQZ \
        | samtools view -uSh - \
        | samtools sort - -o $(basename "${FQZ/\.fq\.gz/}")_MAPPING_bwamem.sort.bam
        ...
        echo -e "bowtie2 \n"
        gzip -dc $FQZ \
        | bowtie2 -p40 -q -x $REF_Bowtie -U /dev/stdin -S $(basename "${FQZ/\.fq\.gz/}")a.sam
        ( samtools view -uSh $(basename "${FQZ/\.fq\.gz/}")a.sam \
        | samtools sort - -o $(basename "${FQZ/\.fq\.gz/}")_MAPPING_bowtie2.sort.bam) &

        #Bowtie2-aDNA
        echo -e "bowtie2aDNA \n"
        gzip -dc $FQZ \
        | bowtie2 -p40 -q --mp 1,1 --ignore-quals --score-min L,0,-0.03 -x $REF_Bowtie -U /dev/stdin -S $(basename "${FQZ/\.fq\.gz/}")b.sam
        ( samtools view -uSh $(basename "${FQZ/\.fq\.gz/}")b.sam \
        | samtools sort - -o $(basename "${FQZ/\.fq\.gz/}")_MAPPING_bowtie2aDNA.sort.bam) &

        #Bowtie2-Vsens
        echo -e "bowtie2VSens \n"
        gzip -dc $FQZ \
        | bowtie2 -p40 -q --very-sensitive -x $REF_Bowtie -U /dev/stdin -S $(basename "${FQZ/\.fq\.gz/}")c.sam
        ( samtools view -uSh $(basename "${FQZ/\.fq\.gz/}")c.sam \
        | samtools sort - -o $(basename "${FQZ/\.fq\.gz/}")_MAPPING_bowtie2Vsens.sort.bam) &
done
wait
