...
echo -ne "FileName\tFragmentSize\tType\tMutation\tMapper\tTotalReads\tMappedReads\t%MappedReads\tCoverage\tDepthCov\n">>Summaries.txt;
for FILE in *.bam; do Col1=$(echo "$FILE"); #Filename
  Col2=$(echo "$FILE" | cut -d"." -f1 | cut -d"_" -f2); #FragmentSize
  Col3=$(echo "$FILE" | cut -d"." -f2); #Type
  Col4=$(echo "$FILE" | cut -d"." -f3 | cut -d"_" -f1); #Mutation
  Col5=$(echo "$FILE" | cut -d"." -f3 | cut -d"_" -f2); #Mapper
